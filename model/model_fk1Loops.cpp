...
double k1;				    	// TF protein translation rate
double k1_iter[]0.001, 0.01, 0.1, 0.3, 0.6, 1};    // TF protein translation rate
...
double f_iter[]{0.001, 0.002, 0.003,0.005, 0.007, 0.008, 0.009, 0.01, 0.02, 0.03, 0.05, 0.07, 0.08, 0.09, 0.1, 0.3, 0.5, 0.6, 0.9, 1, 1.4, 1.7, 8, 9}; // don't run all at once, but chunk up and adapt t_max and dt based on frequency range
...
max_program_counter = (sizeof(f_iter)/sizeof(f_iter[0])) * (sizeof(k1_iter)/sizeof(k1_iter[0]));

// k1 loop
for(i=0; i<(sizeof(k1_iter)/sizeof(k1_iter[0])); i++)
{

	// select k1 for current iteration
	k1 = k1_iter[i];
	...
	for(j=0; j<(sizeof(f_iter)/sizeof(f_iter[0])); j++)
	{
		...
		concat_file_name << fstart << "f-" << frequency << "_d0-" << d0 << "_k1-" << k1;
		...
		file_out.close();
		program_counter++;
		cout << "Finished " << program_counter << " of " << max_program_counter << " combinations; f = " << frequency << ", d0 = " << d0 << endl;
